...
class MainWindow(wx.Frame):
    def __init__(self, parent, title, width, height):
        ...
        menuExit = fileMenu.Append(wx.ID_EXIT, 'Leave', '')

        onlineMenu = wx.Menu()
        menuConnect = onlineMenu.Append(wx.ID_OPEN, 'Join', '')
        menuHost = onlineMenu.Append(wx.ID_REFRESH, 'Hosting', '')

        helpMenu = wx.Menu()
        menuAbout = helpMenu.Append(wx.ID_ABOUT, 'Info', '')
    ...
    def aboutProgram(self, e):
        prompt = wx.MessageDialog(self, 'Created by FeelThatDat', 'About', wx.OK)
        prompt.ShowModal()
        prompt.Destroy()
...
class hostOnline(wx.Frame):
    def __init__(self, parent, mainWindow, title, width, height):
        ...
        self.playerNameBox = wx.TextCtrl(self.hostScreen, value=self.playerName, pos=(self.PLAYERNAMEBOXX, 60),
                                         size=(80, -1))
...
class NewGamePrompt(wx.Frame):
    ...
    def __init__(self, parent, mainWindow, title, width, height):
        ...
        self.numberDropdown = wx.ComboBox(self.newGameScreen, value=self.numberOptions[0], pos=(120, 15), size=(40, -1),
                                          choices=self.numberOptions, style=wx.CB_READONLY)
        ...
        self.playerOneNameBox = wx.TextCtrl(self.newGameScreen, value=self.playerOneName, pos=(self.PLAYERNAMEBOXX, 60),
                                            size=(80, -1))
    ...
    def changePlayers(self, e):
        ...
        if self.numberOfPlayers is 1:
            self.playerOneText = wx.StaticText(self.newGameScreen, label="Player 1's Name ",
                                               pos=(self.PLAYERNAMETEXTX, 62))
            self.playerOneNameBox = wx.TextCtrl(self.newGameScreen, value=self.playerOneName,
                                                pos=(self.PLAYERNAMEBOXX, 60), size=(80, -1))
            self.Bind(wx.EVT_TEXT, self.changePlayerOneName, self.playerOneNameBox)
            ...
            self.playerTwoText = wx.StaticText(self.newGameScreen, label="Player 2's Name ",
                                               pos=(self.PLAYERNAMETEXTX, 92))
            self.playerTwoNameBox = wx.TextCtrl(self.newGameScreen, value=self.playerTwoName,
                                                pos=(self.PLAYERNAMEBOXX, 90), size=(80, -1))
            ...
            self.playerThreeText = wx.StaticText(self.newGameScreen, label="Player 3's Name ",
                                                 pos=(self.PLAYERNAMETEXTX, 122))
            self.playerThreeNameBox = wx.TextCtrl(self.newGameScreen, value=self.playerThreeName,
                                                  pos=(self.PLAYERNAMEBOXX, 120), size=(80, -1))
            ...
            self.playerFourText = wx.StaticText(self.newGameScreen, label="Player 4's Name ",
                                                pos=(self.PLAYERNAMETEXTX, 152))
            self.playerFourNameBox = wx.TextCtrl(self.newGameScreen, value=self.playerFourName,
                                                 pos=(self.PLAYERNAMEBOXX, 150), size=(80, -1))
...
class Table(wx.Panel):
    def __init__(self, parent, numberOfPlayers, playerNames):
        ...
        self.SetBackgroundColour((12, 40, 100))
        self.parent = parent
        self.CARDX = 30
        self.CARDY = 300
        self.CARDWIDTH = 120
    ...
    def renderTable(self, e):
        ...
        if showAnnouncement:
            ...
            for player in self.playerList:
                if player.state is 'out':
                    outPlayerText = wx.StaticText(self, label=player.name + ' ran out of money!',
                                                  pos=(self.CARDX, ANNOUNCEMENTY))
            ...
            if self.playerList[0].name is 'Dealer':
                ...
                for player in self.playerList:
                    endResult = wx.StaticText(self, label=player.name + ' ended with $' + str(player.money) + '.',
                                              pos=(self.CARDX, ANNOUNCEMENTY))
                    endResult.SetFont(self.NAMEFONT)
                    endResult.SetForegroundColour(self.NAMECOLOUR)
                    ANNOUNCEMENTY += self.TEXTSPACING
